...
async function skrinLogin(page) {
    ...
    await goWithRetry.default(page, 'https://sumatera.sitb.id/sitb2024/app', { waitUntil: 'networkidle2' });
    ...
    await puppeteer_utils.waitForDomStable(page, 2000, 10000);
...
async function enterSkriningPage(page, replacePage = true) {
    await goWithRetry.default(page, 'https://sumatera.sitb.id/sitb2024/skrining', {
        waitUntil: 'networkidle2',
        timeout: 120000
    });
    await puppeteer_utils.waitForDomStable(page, 3000, 30000);
    await page.waitForSelector('#btnAdd_ta_skrining', { visible: true });
    await page.click('#btnAdd_ta_skrining');
    if (replacePage) {
        await goWithRetry.default(page, 'https://sumatera.sitb.id/sitb2024/Skrining/add', {
            waitUntil: 'networkidle2',
            timeout: 120000
        });
    }
...
async function autoLoginAndEnterSkriningPage(page) {
    ...
    await goWithRetry.default(page, 'https://sumatera.sitb.id/sitb2024/skrining', { waitUntil: 'networkidle2', timeout: 120000 });
    await puppeteer_utils.waitForDomStable(page, 3000, 30000);
            ...
            if (firstReason === 'captcha_visible') {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        await browser.sleep(1000); // wait before next check
                    }
                }
                if (!solved) {
...
exports.autoLoginAndEnterSkriningPage = autoLoginAndEnterSkriningPage;
