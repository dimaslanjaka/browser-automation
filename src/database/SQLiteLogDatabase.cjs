...
var LogDatabaseHelper = require('../../_virtual/LogDatabaseHelper.cjs');
...
class SQLiteLogDatabase {
    ...
    constructor(dbFileName) {
        ...
        process.on('exit', () => {
            const backupPath = path.join(backupDirectoryPath, `${name}-backup-${LogDatabaseHelper.LogDatabaseHelperExports.getJakartaTimestamp('YYYYMMDD-HHmmss')}.sql`);
    ...
    async backup(destPath) {
        ...
        const child = crossSpawn.spawn('sqlite3', dumpArgs, { stdio: ['ignore', 'pipe', 'inherit'], env });
     ...
     */
    addLog(log) {
        const timestamp = log.timestamp || LogDatabaseHelper.LogDatabaseHelperExports.getJakartaTimestamp('YYYY-MM-DDTHH:mm:ssZ');
        this.db
            .prepare(`INSERT OR REPLACE INTO logs (id, data, message, timestamp) VALUES (?, ?, ?, ?)`)
            .run(log.id, require$$4.jsonStringifyWithCircularRefs(log.data), log.message, timestamp);
    }
    /**
    ...
    getLogById(id) {
        ...
        return {
            id: row.id,
            data: require$$4.jsonParseWithCircularRefs(row.data),
            message: row.message,
