...
class MysqlLogDatabase {
    ...
    constructor(dbName, options = {}) {
        ...
        this.helper = new MySQLHelper.MySQLHelper(mergedOptions);
    }
    async waitReady() {
     ...
     */
    async addLog({ id, data, message, timestamp = undefined }, options = {}) {
        await this.ensureReady();
        if (!timestamp)
            timestamp = LogDatabaseHelper.LogDatabaseHelperExports.getJakartaTimestamp();
        const defaultOptions = {
...
exports.MysqlLogDatabase = MysqlLogDatabase;
