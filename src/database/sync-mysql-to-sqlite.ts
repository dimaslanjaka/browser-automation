...
/**
 * Sync logs from a MySQL table to a SQLite database.
 * Options:
 *  - sqliteName: SQLite database filename (without .db)
 *  - mysqlTable: MySQL table name to read from (reserved for compatibility)
 *  - mysqlDbName: MySQL database name
 *  - dryRun: if true, don't write to SQLite
 *  - concurrency: number of concurrent inserts
 *  - remove: if true, remove rows from MySQL after successful insert
 */
export async function syncMySQLToSqlite({
  ...
  try {
    const logs = await mysqlDb.getLogs();
    if (!logs || logs.length === 0) {
      console.log(`[${syncLabel}] No logs found to migrate in MySQL database: ${mysqlDbName}.`);
      return { migrated: 0 };
    }
    ...
    for (let i = 0; i < logs.length; i += concurrency) {
      ...
      const promises = batch.map(async (log) => {
        ...
        try {
          sqliteDb.addLog(log);
          migrated += 1;
          if (remove) await mysqlDb.removeLog(log.id);
          return true;
        } catch (err) {
...
// CLI usability
if (process.argv[1] && process.argv[1].includes('sync-mysql-to-sqlite')) {
  (async () => {
