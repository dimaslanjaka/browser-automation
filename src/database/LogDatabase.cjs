...
var MysqlLogDatabase = require('./MysqlLogDatabase.cjs');
var SQLiteLogDatabase = require('./SQLiteLogDatabase.cjs');
var isDebug = require('../../_virtual/isDebug.cjs');
...
class LogDatabase {
    ...
    constructor(dbName, options) {
        ...
        this.pref = new require$$4.SharedPreferences({ namespace: this.constructor.name });
        this.sqliteDbPath = SQLiteLogDatabase.getDatabaseFilePath(this.dbName);
    }
    async getType() {
        if (!this.store)
            await this.initialize();
        const optionsType = this.options.type || 'sqlite';
        return { optionsType, classType: this.store instanceof MysqlLogDatabase.MysqlLogDatabase ? 'mysql' : 'sqlite' };
    }
    ...
    async waitReady() {
        if (!this.store)
            await this.initialize();
        if (this.store instanceof MysqlLogDatabase.MysqlLogDatabase) {
    ...
    async close() {
        ...
        if (this.store instanceof SQLiteLogDatabase.SQLiteLogDatabase || this.pref.getBoolean('needsMigration', false)) {
            try {
                await this.migrate();
            }
            catch (e) {
    ...
    async initialize() {
        ...
        if (this.options?.type === 'sqlite') {
            this.store = new SQLiteLogDatabase.SQLiteLogDatabase(this.dbName);
        }
        else if (this.options?.type === 'mysql') {
            this.store = new MysqlLogDatabase.MysqlLogDatabase(this.dbName, this.options);
        }
        else {
            // auto
            try {
    ...
    async showProcessList(print = false) {
        ...
        if (print)
            console.log('showProcessList is only available for MySQL databases');
    }
    /**
     ...
     */
    async isChecksumChanged(save = false) {
        const sqliteFile = SQLiteLogDatabase.getDatabaseFilePath(this.dbName);
        const sqliteChecksum = await require$$4.getChecksum(sqliteFile);
    ...
    async migrate() {
        ...
        // Perform migration
        const sqlite = new SQLiteLogDatabase.SQLiteLogDatabase(this.dbName);
        const mysql = new MysqlLogDatabase.MysqlLogDatabase(this.dbName);
