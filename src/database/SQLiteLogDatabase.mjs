...
import { L as LogDatabaseHelperExports } from '../../_virtual/LogDatabaseHelper.mjs';
...
class SQLiteLogDatabase {
    ...
    constructor(dbFileName) {
        ...
        process.on('exit', () => {
            const backupPath = path.join(backupDirectoryPath, `${name}-backup-${LogDatabaseHelperExports.getJakartaTimestamp('YYYYMMDD-HHmmss')}.sql`);
     ...
     */
    addLog(log) {
        const timestamp = log.timestamp || LogDatabaseHelperExports.getJakartaTimestamp('YYYY-MM-DDTHH:mm:ssZ');
        this.db
...
export { SQLiteLogDatabase, getDatabaseFilePath };
