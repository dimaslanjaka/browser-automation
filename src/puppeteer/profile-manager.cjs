 ...
 */
const GLOBAL_PUPPETEER_DIR = path.join(os.tmpdir(), 'browser-automation-puppeteer');
/**
 ...
 */
const GLOBAL_PROFILES_DIR = path.join(GLOBAL_PUPPETEER_DIR, 'profiles');
/**
...
function isProfileInUseLaunchError(error) {
    ...
    return (errorMessage.includes('already running for') ||
 ...
 * @returns {Promise<any>}
 */
async function launchWithProfileFallback({ launchFn, launchOptions, autoSwitchProfileDir = true, launcherName, maxFallbackLaunchAttempts = 10 }) {
    ...
    while (true) {
        try {
            ...
            const fallbackUserDataDir = reserveNextFallbackProfileDir(excludedUserDataDirs, 1);
            launchAttempt += 1;
            console.warn(`${launcherName} launch failed because userDataDir is busy, retrying with profile: ${fallbackUserDataDir}`);
 ...
 * @returns {string}
 */
function reserveClusterUserDataDir({ preferredUserDataDir, reservedUserDataDirs, autoSwitchProfileDir = true, fallbackProfileStartIndex = 1 }) {
...
exports.GLOBAL_PUPPETEER_DIR = GLOBAL_PUPPETEER_DIR;
exports.getFallbackProfileDir = getFallbackProfileDir;
exports.isUserDataDirInUse = isUserDataDirInUse;
exports.launchWithProfileFallback = launchWithProfileFallback;
exports.prepareLaunchOptionsWithProfileFallback = prepareLaunchOptionsWithProfileFallback;
exports.reserveClusterUserDataDir = reserveClusterUserDataDir;
exports.reserveNextFallbackProfileDir = reserveNextFallbackProfileDir;
