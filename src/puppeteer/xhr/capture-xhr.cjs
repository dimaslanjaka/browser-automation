...
function setupXhrCapture(page, opts) {
    ...
    async function getPostDataSafe(request) {
        const _req = request;
        let rawPostData = undefined;
        if (typeof _req.fetchPostData === 'function')
            rawPostData = _req.fetchPostData();
        else if (typeof _req.postData === 'function')
            rawPostData = _req.postData();
        else
            rawPostData = undefined;
        if (rawPostData === undefined || rawPostData === null)
            return undefined;
        ...
        if (typeof rawPostData === 'string')
            return rawPostData;
        if (typeof rawPostData.toString === 'function')
    ...
    const responseHandler = async (response) => {
        try {
            ...
            if (!request || request.resourceType() !== 'xhr')
                return;
...
exports.setupXhrCapture = setupXhrCapture;
