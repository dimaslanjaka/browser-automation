...
var PageContext = require('./PageContext.cjs');
 ...
 */
async function createContext(page, iframeSelector) {
    if (!iframeSelector) {
        return PageContext.PageContext.fromPage(page);
    }
    ...
    return PageContext.PageContext.fromFrame(frame, page);
}
