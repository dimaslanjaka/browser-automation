...
var directProcessData = require('../../runner/skrin/direct-process-data.cjs');
...
async function getPage() {
    ...
    while (true) {
        ...
        try {
            const browser$1 = await puppeteer.connect({
                browserWSEndpoint: endpoint,
                protocolTimeout: 180_000
            });
            browser$1.once('disconnected', async () => {
                release();
            ...
            await goWithRetry.default(page, 'http://www.webmanajemen.com', {
                timeout: 10000,
...
async function parallelSkrin(opts) {
    ...
    if (cliSkipValidateDb) {
        dataKunto = await index.loadCsvData();
    }
    else {
        dataKunto = await Bluebird.filter(await index.loadCsvData(), async (data) => {
    ...
    async function processOne(data) {
        return directProcessData.processData(page, data, database, options).catch((err) => ({
            status: 'error',
    ...
    if (opts.loop) {
        ...
        let processed = 0;
        for (const data of require$$4.array_shuffle(dataKunto)) {
            if (processed >= max)
                break;
            processed++;
        ...
        const dataToProcess = opts.randomize ? require$$4.array_shuffle(dataKunto) : dataKunto;
...
exports.default = parallelSkrin;
exports.parallelSkrin = parallelSkrin;
