...
function getFingerprintDimensions(parsedObj) {
    ...
    const w = number.toNumber(screenData?.screen?.width ?? screenData?.real?.width ?? screenData?.css?.deviceWidth);
    const h = number.toNumber(screenData?.screen?.height ?? screenData?.real?.height ?? screenData?.css?.deviceHeight);
    const viewportW = number.toNumber(screenData?.viewport?.width);
    const viewportH = number.toNumber(screenData?.viewport?.height);
    const availW = number.toNumber(screenData?.available?.width);
    const availH = number.toNumber(screenData?.available?.height);
    return {
        width: w ?? null,
        height: h ?? null,
...
function fingerprintMatchesSize(parsedObj, sizeOptions) {
    ...
    if (w == null && h == null && vpW == null && vpH == null)
        return false;
    if (sizeOptions.width != null) {
        const target = number.toNumber(sizeOptions.width);
        if ((w != null && w !== target) || (vpW != null && vpW !== target))
            return false;
    }
    if (sizeOptions.height != null) {
        const target = number.toNumber(sizeOptions.height);
        if ((h != null && h !== target) || (vpH != null && vpH !== target))
            return false;
    }
    if (sizeOptions.minWidth != null) {
        const min = number.toNumber(sizeOptions.minWidth);
        if ((w != null && w < min) || (vpW != null && vpW < min) || (avW != null && avW < min))
            return false;
    }
    if (sizeOptions.minHeight != null) {
        const min = number.toNumber(sizeOptions.minHeight);
        if ((h != null && h < min) || (vpH != null && vpH < min) || (avH != null && avH < min))
            return false;
    }
    if (sizeOptions.maxWidth != null) {
        const max = number.toNumber(sizeOptions.maxWidth);
        if ((w != null && w > max) || (vpW != null && vpW > max) || (avW != null && avW > max))
            return false;
    }
    if (sizeOptions.maxHeight != null) {
        const max = number.toNumber(sizeOptions.maxHeight);
        if ((h != null && h > max) || (vpH != null && vpH > max) || (avH != null && avH > max))
            return false;
    }
    return true;
}
...
function saveFingerprintToCache(fingerprint, tags = []) {
    ...
    require$$4.writefile(fingerprintCacheFilePath, fingerprint);
...
exports.fetchAndSaveFingerprintToCache = fetchAndSaveFingerprintToCache;
exports.getFingerprintCacheDir = getFingerprintCacheDir;
exports.getLatestCachedFingerprint = getLatestCachedFingerprint;
exports.getRandomCachedFingerprint = getRandomCachedFingerprint;
exports.listCachedFingerprintFiles = listCachedFingerprintFiles;
exports.parseScreenSize = parseScreenSize;
exports.saveFingerprintToCache = saveFingerprintToCache;
