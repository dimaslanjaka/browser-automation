...
var getStreetAddressInformation = require('./getStreetAddressInformation.cjs');
var normalizeAddressNameToIndonesian = require('./normalizeAddressNameToIndonesian.cjs');

const normalizeAddressPart = (value) => normalizeAddressNameToIndonesian.normalizeAddressNameToIndonesian(value);
...
const createKelurahanResult = ({ nik, provinsi, kotakab, namaKec, kelurahanName, address, geocoded, source, id, error, available }) => {
 ...
 */
async function findExactKelurahanFromAddress(nikData, address, options = {}) {
    if (!nikData || !address) {
        return null;
    }
    ...
    try {
        // Geocode the address
        const geocoded = await getStreetAddressInformation.getStreetAddressInformation(keyword, options);
        ...
        exactMatch = kelurahan.find((k) => {
            ...
            return (addressUpperCase.includes(kelurahanUpper) ||
...
exports.findExactKelurahanFromAddress = findExactKelurahanFromAddress;
