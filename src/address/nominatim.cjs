...
const __filename$1 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('src/address/nominatim.cjs', document.baseURI).href)));
...
async function geocodeWithNominatim(keyword, options = {}) {
    ...
    try {
        ...
        const response = await axios.get(baseURL, { ...(await utils.axiosConfigBuilder(options)), params });
