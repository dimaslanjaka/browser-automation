...
async function geoCodeWithGeoapify(keyword, apiKey, options = {}) {
    ...
    const resolvedApiKey = require$$4.isEmpty(apiKey, { allowWhitespace: true }) ? getGeoapifyKeys()[0] : apiKey;
    if (require$$4.isEmpty(resolvedApiKey, { allowWhitespace: true })) {
        console.error('Geoapify error: API key is required (pass apiKey or set GEOAPIFY_KEYS)');
        return null;
    }
    const cacheFile = utils.getCacheFilePath(keyword);
    ...
    try {
        ...
        if (!options.noCache) {
            ...
            if (cached) {
                if (verbose)
                    console.log('Geoapify cache hit:', cacheFile);
                return JSON.parse(cached);
            }
            else {
                if (verbose)
                    console.log('Geoapify cache miss:', cacheFile);
            }
        }
        else {
            if (verbose)
                console.log('Geoapify noCache option set, bypassing cache');
        }
    }
    catch {
    ...
    try {
        const response = await axios.get(url.toString(), { ...(await utils.axiosConfigBuilder(options)) });
        const data = response.data;
        if (require$$4.isEmpty(data) || require$$4.isEmpty(data.features, { allowWhitespace: true })) {
            if (verbose)
                console.log('Geoapify no results for keyword:', keyword);
            return null;
        }
        ...
        if (require$$4.isEmpty(fullAddress)) {
        ...
        if (response.status === 200) {
            ...
            require$$4.writefile(cacheFile, result);
            if (verbose)
                console.log('Geoapify response cached:', cacheFile);
        }
...
exports.geoCodeWithGeoapify = geoCodeWithGeoapify;
exports.getGeoapifyKeys = getGeoapifyKeys;
