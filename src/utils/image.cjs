    ...
    try {
        ...
        const cmd = process.platform === 'win32'
            ...
            : process.platform === 'darwin'
                ? `open "${safePath}"`
                : `xdg-open "${safePath}"`;
        require$$0.exec(cmd, (err) => {
            if (err)
                console.error('Failed to open image:', err && err.message ? err.message : err);
        });
    }
    catch (e) {
        // ignore
...
exports.imageToUri = imageFileToDataUrl;
exports.openImageExternally = openImageExternally;
