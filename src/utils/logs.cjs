...
function requireLogs () {
	...
	hasRequiredLogs = 1;
	const ansiColors$1 = ansiColors;
	const fs$1 = fs;
	const nikParse = require$$2;
	const path$1 = path;
	const { readfile } = require$$4;
	const defaultLogFilePath = path$1.join(process.cwd(), '.cache/lastData.log');
	/**
	...
	function appendLog(data, message = 'Processed Data', logFilePath = null) {
	    ...
	    fs$1.appendFileSync(logFilePath, logEntry, 'utf8');
	}
	/**
	...
	function getLogData(logFilePath = null) {
	    ...
	    return log
	        ...
	        if (!data || typeof data !== 'object') {
	            throw new Error(`Invalid data format at line ${log.split('\n').indexOf(line)}: "${line}" (Log path: ${logFilePath})`);
	        }
	        if (data.error) {
	            throw new Error(`Error parsing log line at index ${log.split('\n').indexOf(line)}: ${data.error}. Line: "${line}" (Log path: ${logFilePath})`);
	        }
	...
	function colorizeJson(value, indent = 2, level = 0) {
	    ...
	    if (typeof value === 'string') {
	        return ansiColors$1.yellow('"' + value + '"');
	    }
	    if (typeof value === 'number') {
	        return ansiColors$1.magenta(value);
	    }
	    if (typeof value === 'boolean') {
	        return ansiColors$1.blue(value);
	    }
	    return String(value);
	}
	/**
	...
	function logInline(...args) {
	    ...
	    if (args.length > 1) {
	        output = args
	            ...
	            if (arg === null) {
	                ...
	                try {
	                    ...
	                    return ansiColors$1.red('[Unserializable Object]');
	                }
	            }
	            else if (typeof arg === 'number') {
	                return ansiColors$1.magenta(arg);
	            }
	            else if (typeof arg === 'boolean') {
	                return ansiColors$1.blue(arg);
	            }
	            return String(arg);
	        })
	            .join(' ');
	        ...
	        if (message === null) {
	            ...
	            try {
	                ...
	                output = ansiColors$1.red('[Unserializable Object]');
	            }
	        }
	        else if (typeof message === 'number') {
	            output = ansiColors$1.magenta(message);
	        }
	        else if (typeof message === 'boolean') {
	            output = ansiColors$1.blue(message);
	        }
	        else {
	    ...
	    _commonjsHelpers.commonjsGlobal.__lastLogWasInline = true;
	}
	/**
	...
	function logLine(...args) {
	    ...
	    const prefix = _commonjsHelpers.commonjsGlobal.__lastLogWasInline ? '\n' : '';
	    process.stdout.write(`${prefix}${output}\n`);
	    _commonjsHelpers.commonjsGlobal.__lastLogWasInline = false;
	}
	logs = { logLine, logInline, getLogData, appendLog, defaultLogFilePath };
	return logs;
}

exports.__require = requireLogs;
