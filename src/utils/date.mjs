 ...
 */
function getDatesWithoutSundays(monthName, year = new Date().getFullYear(), format = 'YYYY-MM-DD', limitToToday = false) {
    const monthMap = {
        january: 0,
        februari: 1,
        february: 1,
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (limitToToday && date > today)
            break; // Stop at today if limit enabled
...
function enforceDateFormat(dateStr, formats, context = '') {
    ...
    if (!parsed.isValid()) {
        throw new Error(`❌ Invalid birth date format${context ? ` in ${context}` : ''}. Expected one of [${formats.join(', ')}], got: ${dateStr}`);
    }
...
export { containsMonth, parseDate as dateStringToDDMMYYYY, enforceDateFormat, extractMonthName, getAge, getAgeFromDateString, getDatesWithoutSundays, parseDate };
