...
import { toValidMySQLDatabaseName } from '../../database/db_utils.mjs';

const DEFAULT_CACHE_DIR = path.join(process.cwd(), 'tmp/fixdata-cache', toValidMySQLDatabaseName(process.env.MYSQL_DBNAME || process.env.DATABASE_FILENAME || 'default'));
/**
...
class FixDataCache {
    ...
    async clear(options = {}) {
        ...
        try {
            ...
            if (options.verbose)
                console.log('[FixDataCache] Cache directory emptied');
        }
        catch (err) {
    ...
    async clearEntry(data, options = {}) {
        ...
        try {
            if (await fs.pathExists(file)) {
                ...
                if (options.verbose)
                    console.log(`[FixDataCache] Removed cache file: ${path.basename(file)}`);
            }
        }
        catch (err) {
...
export { DEFAULT_CACHE_DIR, FixDataCache };
