...
import { getNumbersOnly } from '../browser.mjs';
import { loadCsvData } from '../../../data/index.mjs';
import { fixNama } from './fixData/nama.mjs';
import { fixPekerjaan } from './fixData/pekerjaan.mjs';
import { fixAlamat } from './fixData/alamat.mjs';
import { fixBirth } from './fixData/birth.mjs';
import { fixTanggalEntry } from './fixData/tanggal-entry.mjs';
import { fixTbBb } from './fixData/tb-bb.mjs';
import { FixDataCache } from './FixDataCache.mjs';
...
async function fixData(data, options = {
    ...
    if (!normalizedNik) {
        throw new Error(`Invalid NIK length: ${getNumbersOnly(nik)} (expected 16 characters)\n\n${JSON.stringify(initialData, null, 2)}`);
    }
    nik = normalizedNik;
...
export { fixData as default, isValidNik, normalizeNik };
