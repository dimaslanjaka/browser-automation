...
var require$$2 = require('nik-parser-jurusid');
var browser = require('../browser.cjs');
var index = require('../../../data/index.cjs');
var nama = require('./fixData/nama.cjs');
var pekerjaan = require('./fixData/pekerjaan.cjs');
var alamat = require('./fixData/alamat.cjs');
var birth = require('./fixData/birth.cjs');
var tanggalEntry = require('./fixData/tanggal-entry.cjs');
var tbBb = require('./fixData/tb-bb.cjs');
var FixDataCache = require('./FixDataCache.cjs');

function normalizeNik(rawNik) {
    if (!rawNik)
        return null;
    const digits = browser.getNumbersOnly(rawNik);
...
async function fixData(data, options = {
    ...
    if (!initialData)
        throw new Error(`Invalid data format: data is required\n\n${JSON.stringify(initialData, null, 2)}`);
    // Normalize key fields for cache lookup
    let nik = initialData.NIK || initialData.nik || null;
    let nama$1 = initialData.NAMA || initialData.nama || null;
    if (!nik || !nama$1)
    ...
    if (!normalizedNik) {
        throw new Error(`Invalid NIK length: ${browser.getNumbersOnly(nik)} (expected 16 characters)\n\n${JSON.stringify(initialData, null, 2)}`);
    }
    nik = normalizedNik;
    // Initialize cache with custom directory if provided
    const cache = new FixDataCache.FixDataCache(options.cacheDir);
    ...
    initialData.NIK = nik;
    // Normalize nama
    const namaResult = nama.fixNama(initialData, { verbose: options.verbose, fixNamaBayi: options.fixNamaBayi === true });
    initialData.nama = namaResult.nama;
    initialData.NAMA = namaResult.nama;
    // Parse NIK
    const parsed_nik = require$$2.nikParserStrictSync(nik);
    ...
    const tanggalResult = tanggalEntry.fixTanggalEntry(initialData, {
    ...
    let age = 0;
    const birthResult = birth.fixBirth(initialData, { verbose: options.verbose });
    age = birthResult.age;
    initialData.age = age;
    // Gender
    ...
    const pekerjaanResult = pekerjaan.fixPekerjaan(initialData, { verbose: options.verbose });
    ...
    // Fix alamat
    let alamat$1 = initialData.alamat || initialData.ALAMAT || null;
    const alamatResult = await alamat.fixAlamat(alamat$1, initialData.parsed_nik, nik, initialData, data, {
        verbose: options.verbose
    });
    initialData.alamat = alamatResult.alamat;
    initialData.ALAMAT = alamatResult.alamat;
    // Fix tinggi and berat badan
    const tbBbResult = tbBb.fixTbBb(initialData, { verbose: options.verbose });
...
if (process.argv[1].includes('fixData.js')) {
    (async () => {
        try {
            ...
            const dataKunto = await index.loadCsvData();
...
exports.normalizeNik = normalizeNik;
