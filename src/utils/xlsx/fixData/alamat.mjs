...
import { geocodeWithNominatim } from '../../../address/nominatim.mjs';
import 'bluebird';
...
async function fixAlamat(alamat, parsedNik, nik, initialData, dataRef, options = { verbose: false }) {
    if (!alamat) {
        if (parsedNik && parsedNik.status === 'success') {
            ...
            alamat = [parsed_data.kelurahan?.[0]?.name, parsed_data.namaKec, parsed_data.kotakab, parsed_data.provinsi]
                ...
                .join(', ');
            if (options.verbose)
                waitEnterExports.logLine(`${ansiColors.cyan('[fixAlamat]')} Alamat from parsed NIK: ${alamat}`);
            ...
            if (kotakab.length === 0 || namaKec.length === 0 || provinsi.length === 0) {
                ...
                if (options.verbose)
                    waitEnterExports.logLine(`${ansiColors.cyan('[fixAlamat]')} Fetching address from Nominatim for: ${keywordAddr}`);
                if (options.verbose)
                    waitEnterExports.logLine(`${ansiColors.cyan('[fixAlamat]')} Nominatim result:`, address);
    ...
    if (initialData.parsed_nik && initialData.parsed_nik.status === 'success') {
        ...
        if (/kota adm\.?/i.test(parsed_data.kotakab.toLowerCase())) {
            ...
            if (options.verbose) {
                waitEnterExports.logLine(`${ansiColors.cyan('[fixAlamat]')} Normalized kotakab: ${parsed_data.kotakab}`);
            }
        }
        initialData.parsed_nik.data = parsed_data; // Update parsed_nik with new data
    }
    if (options.verbose)
        waitEnterExports.logLine(`${ansiColors.cyan('[fixAlamat]')} Alamat fixed: ${alamat}`);
