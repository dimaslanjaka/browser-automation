...
function fixPekerjaan(initialData, options = { verbose: false }) {
    ...
    if (!pekerjaan) {
        ...
        if (options.verbose)
            waitEnter.waitEnterExports.logLine(`${ansiColors.cyan('[fixPekerjaan]')} Pekerjaan: ${pekerjaan}`);
    }
    ...
    for (const { pattern, value } of jobMappings) {
        if (pattern.test(pekerjaan.toLowerCase())) {
            pekerjaan = value;
            if (options.verbose)
                waitEnter.waitEnterExports.logLine(`${ansiColors.cyan('[fixPekerjaan]')} Pekerjaan mapped: ${pekerjaan}`);
            break;
        }
    }
    ...
    if (options.verbose)
        waitEnter.waitEnterExports.logLine(`${ansiColors.cyan('[fixPekerjaan]')} Pekerjaan fixed: ${pekerjaan}`);
    initialData.pekerjaan = pekerjaan;
    initialData.PEKERJAAN = pekerjaan;
    return { pekerjaan, initialData, pekerjaan_original: initialData.pekerjaan };
}

exports.fixPekerjaan = fixPekerjaan;
