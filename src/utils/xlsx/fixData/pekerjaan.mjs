...
function fixPekerjaan(initialData, options = { verbose: false }) {
    ...
    if (!pekerjaan) {
        ...
        if (options.verbose)
            waitEnterExports.logLine(`${ansiColors.cyan('[fixPekerjaan]')} Pekerjaan: ${pekerjaan}`);
    }
    ...
    for (const { pattern, value } of jobMappings) {
        if (pattern.test(pekerjaan.toLowerCase())) {
            pekerjaan = value;
            if (options.verbose)
                waitEnterExports.logLine(`${ansiColors.cyan('[fixPekerjaan]')} Pekerjaan mapped: ${pekerjaan}`);
            break;
        }
    }
    ...
    if (options.verbose)
        waitEnterExports.logLine(`${ansiColors.cyan('[fixPekerjaan]')} Pekerjaan fixed: ${pekerjaan}`);
