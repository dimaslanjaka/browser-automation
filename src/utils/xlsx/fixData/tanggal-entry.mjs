...
import { getDatesWithoutSundays, extractMonthName } from '../../date.mjs';
import 'bluebird';
...
function fixTanggalEntry(initialData, options = { verbose: false, autofillTanggalEntry: false }) {
    ...
    if (String(tanggalEntry).trim().length === 0) {
        if (options.autofillTanggalEntry) {
            ...
            if (options.verbose)
                waitEnterExports.logLine(`${ansiColors.cyan('[fixTanggalEntry]')} Generated new tanggal entry for missing value: ${tanggalEntry}`);
        }
        else {
    ...
    if (!moment(tanggalEntry, 'DD/MM/YYYY', true).isValid()) {
        if (typeof tanggalEntry === 'string' &&
            ...
            if (options.verbose)
                waitEnterExports.logLine(`${ansiColors.cyan('[fixTanggalEntry]')} Generated new date for "${tanggalEntry}" from month name in entry: ${tanggalEntry}`);
        }
    ...
    if (options.verbose)
        waitEnterExports.logLine(`${ansiColors.cyan('[fixTanggalEntry]')} Tanggal entry fixed: ${tanggalEntry}`);
    initialData.tanggal = tanggalEntry;
    initialData['TANGGAL ENTRY'] = tanggalEntry;
    return { tanggalEntry, initialData };
}

export { fixTanggalEntry };
