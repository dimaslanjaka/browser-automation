...
function fixBirth(initialData, options = { verbose: false }) {
    ...
    if (tglLahir) {
        if (typeof tglLahir === 'number') {
            ...
            waitEnterExports.logLine(`${ansiColors.cyan('[fixBirth]')} Converted Excel serial date to: ${tglLahir}`);
        }
            ...
            if (parsed_nik &&
                ...
                if (options.verbose) {
                    waitEnterExports.logLine(`${ansiColors.cyan('[fixBirth]')} Corrected TGL LAHIR from NIK: ${parsed_nik.data.lahir}`);
                }
                tglLahir = parsed_nik.data.lahir;
                if (options.verbose)
                    waitEnterExports.logLine(`${ansiColors.cyan('[fixBirth]')} Corrected TGL LAHIR from NIK: ${tglLahir}`);
            }
            else {
    ...
    if (birthDate) {
        ...
        if (!moment(birthDate, 'DD/MM/YYYY', true).isValid()) {
            if (parsed_nik &&
                ...
                if (options.verbose)
                    waitEnterExports.logLine(`${ansiColors.cyan('[fixBirth]')} Age from NIK: ${age} years`);
            }
        }
        age = getAge(birthDate, 'DD/MM/YYYY');
        if (options.verbose)
            waitEnterExports.logLine(`${ansiColors.cyan('[fixBirth]')} Age from TGL LAHIR: ${age} years`);
    }
    ...
    if (options.verbose)
        waitEnterExports.logLine(`${ansiColors.cyan('[fixBirth]')} Birth date and age fixed: ${tglLahir}, Age: ${age} years`);
