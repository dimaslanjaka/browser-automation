...
var skrinUtils = require('../../../runner/skrin-utils.cjs');
require('bluebird');
...
function fixNama(initialData, options = { verbose: false, fixNamaBayi: false }) {
    ...
    if (options.verbose)
        waitEnter.waitEnterExports.logLine(`${ansiColors.cyan('[fixNama]')} Normalized nama: ${normalizedNama}`);
    // Fix bayi substring (if enabled)
    if (options.fixNamaBayi && /bayi/i.test(normalizedNama)) {
        const namaBayi = skrinUtils.parseBabyName(normalizedNama);
        if (namaBayi) {
            if (options.verbose)
                console.log(`Parsed baby name: "${normalizedNama}" -> "${namaBayi}"`);
            normalizedNama = namaBayi;
        }
        else {
            if (options.verbose)
                console.log(`Failed to parse baby name: "${normalizedNama}"`);
        }
        // Verify that the name was successfully parsed
        if (/bayi/i.test(normalizedNama)) {
            throw new Error(`Failed to parse baby name from NAMA: ${normalizedNama}\n\n${JSON.stringify(initialData, null, 2)}`);
        }
    }
