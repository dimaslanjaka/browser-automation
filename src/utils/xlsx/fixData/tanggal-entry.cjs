...
function fixTanggalEntry(initialData, options = { verbose: false, autofillTanggalEntry: false }) {
    ...
    if (String(tanggalEntry).trim().length === 0) {
        if (options.autofillTanggalEntry) {
            ...
            tanggalEntry = require$$4.array_random(date.getDatesWithoutSundays(currentMonth, currentYear, 'DD/MM/YYYY', true));
            if (options.verbose)
                waitEnter.waitEnterExports.logLine(`${ansiColors.cyan('[fixTanggalEntry]')} Generated new tanggal entry for missing value: ${tanggalEntry}`);
        }
        else {
    ...
    if (!moment(tanggalEntry, 'DD/MM/YYYY', true).isValid()) {
        if (typeof tanggalEntry === 'string' &&
            /\b(jan(uari)?|feb(ruari)?|mar(et)?|apr(il)?|mei|jun(i|e)?|jul(i|y)?|agu(stus)?|aug(ust)?|sep(tember)?|okt(ober)?|oct(ober)?|nov(ember)?|des(ember)?|dec(ember)?|bln\s+\w+|bulan\s+\w+)\b/i.test(tanggalEntry)) {
            const monthName = date.extractMonthName(tanggalEntry);
            if (!monthName)
                throw new Error(`Month name not found in tanggalEntry: ${tanggalEntry}\n\n${JSON.stringify(initialData, null, 2)}`);
            tanggalEntry = require$$4.array_random(date.getDatesWithoutSundays(monthName, moment().year(), 'DD/MM/YYYY', true));
            if (options.verbose)
                waitEnter.waitEnterExports.logLine(`${ansiColors.cyan('[fixTanggalEntry]')} Generated new date for "${tanggalEntry}" from month name in entry: ${tanggalEntry}`);
        }
    ...
    if (options.verbose)
        waitEnter.waitEnterExports.logLine(`${ansiColors.cyan('[fixTanggalEntry]')} Tanggal entry fixed: ${tanggalEntry}`);
    initialData.tanggal = tanggalEntry;
    initialData['TANGGAL ENTRY'] = tanggalEntry;
    return { tanggalEntry, initialData };
}

exports.fixTanggalEntry = fixTanggalEntry;
