...
import { nikParser } from '../../../packages/nik-parser/dist/esm/region_controller_async.mjs';
import { isValidNIK } from '../../../packages/nik-parser/dist/esm/nik_validator.mjs';
import path from 'path';
import { writefile, isEmpty } from 'sbg-utility';
import { findExactKelurahanFromAddress } from '../../address/findExactKelurahanFromAddress.mjs';
import '../../address/geoapify.mjs';
import '../../address/nominatim.mjs';
import { waitForDomStable, typeAndTrigger, isElementExist, isElementVisible } from '../../puppeteer_utils.mjs';
import { autoLoginAndEnterSkriningPage } from '../../skrin_puppeteer.mjs';
import { FileLockHelper } from '../../utils/FileLockHelper.mjs';
import { getNumbersOnly, extractNumericWithComma, sleep } from '../../utils/browser.mjs';
import { w as waitEnterExports } from '../../../_virtual/waitEnter.mjs';
import '../../../_virtual/logs.mjs';
import '../../utils/beep.mjs';
import 'moment-timezone';
import { ucwords, findInArray } from '../../utils/string.mjs';
import 'glob';
import 'node:crypto';
import 'node:fs';
import 'node:path';
import fixData from '../../utils/xlsx/fixData.mjs';
import { confirmIdentityModal } from './confirmIdentityModal.mjs';
import { setDatepickerValue, selectDateWithUI } from './datePicker.mjs';
import { getNormalizedFormValues } from './getNormalizedFormValues.mjs';
import { getPersonInfo } from './getPersonInfo.mjs';
import { isIdentityModalVisible } from './isIdentityModalVisible.mjs';
import { isInvalidAlertVisible } from './isInvalidAlertVisible.mjs';
import { isNikErrorVisible } from './isNikErrorVisible.mjs';
import { isNIKNotFoundModalVisible } from './isNIKNotFoundModalVisible.mjs';
import { isSessionExpiredAlertVisible } from './isSessionExpiredAlertVisible.mjs';
import { isSuccessNotificationVisible } from './isSuccessNotificationVisible.mjs';
import removeDuplicateKeys from '../../../_virtual/removeDuplicateKeys.mjs';
...
async function processData(page, data, database, options = {
    ...
    const parsedNik = fixedData.parsed_nik || (await nikParser(fixedData.nik));
    ...
    try {
        ...
        if (await isNikErrorVisible(page)) {
            await waitEnterExports.waitEnter('Please check NIK error notification. Press Enter to continue...');
        ...
        if (fixedData.batuk) {
            ...
            if (/\d/m.test(keteranganBatuk)) {
                ...
                await waitEnterExports.waitEnter('Please fix data batuk/demam. Press Enter to continue...');
            }
            else {
        ...
        while (await isIdentityModalVisible(page)) {
            ...
            await sleep(1000);
            if (await isIdentityModalVisible(page)) {
                await waitEnterExports.waitEnter('Please check identity modal. Press Enter to continue...');
            }
        }
        ...
        while (invalidAlert.result) {
            ...
            if (invalidAlert.result) {
                ...
                await waitEnterExports.waitEnter('Please review the alert and press Enter to continue...');
            }
        }
        ...
        if (sessionExpiredAlertVisible) {
            ...
            await waitEnterExports.waitEnter('Session expired. Please log in again, then press Enter to continue...');
            await autoLoginAndEnterSkriningPage(page);
            return {
                status: 'error',
        ...
        if (isAllowedToSubmit) {
            ...
            try {
                ...
                await waitEnterExports.waitEnter('Failed to click #yesButton for confirmation modal. Please click the button manually, then press Enter to continue...');
            }
            await sleep(1000);
        }
        else {
            console.warn('⚠️\tData processed but not submitted:', fixedData, '\n');
            await waitEnterExports.waitEnter('Please manually submit the form, then press Enter to continue...');
        }
