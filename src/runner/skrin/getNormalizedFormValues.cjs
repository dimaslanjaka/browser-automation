...
async function getNormalizedFormValues(context, containerSelector = '#main-container') {
    const normalize = (formItems) => formItems
        .map((item) => {
    ...
    return normalize(await puppeteer_utils.getFormValues(context, containerSelector));
}

exports.getNormalizedFormValues = getNormalizedFormValues;
