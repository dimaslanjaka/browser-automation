import { dateStringToDDMMYYYY as parseDate, getAge } from '../../utils/date.mjs';
...
async function getPersonInfo(page) {
    ...
    const rawBirthDate = await page.evaluate(() => document.querySelector('input[name="dt_tgl_lahir"]')?.value);
    ...
    const bodyWeight = await page.evaluate(() => document.querySelector('#field_item_berat_badan input[type="text"]')?.value);
    const bodyHeight = await page.evaluate(() => document.querySelector('#field_item_tinggi_badan input[type="text"]')?.value);
