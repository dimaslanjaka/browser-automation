...
function parseDate(dateStr) {
  ...
  for (const { pattern, regex } of uniqueFormats) {
    ...
    if (match2) {
      const parsed = moment__default.default(dateStr, pattern, true);
      if (parsed.isValid()) {
  ...
  if (match) {
    ...
    if (firstNum > 12) {
      const parsed = moment__default.default(dateStr, "DD/MM/YYYY", true);
      if (parsed.isValid() && parsed.year() >= 1900 && parsed.year() <= (/* @__PURE__ */ new Date()).getFullYear() + 10) {
        return parsed.format("DD/MM/YYYY");
      }
    }
    if (secondNum > 12) {
      const parsed = moment__default.default(dateStr, "MM/DD/YYYY", true);
      if (parsed.isValid() && parsed.year() >= 1900 && parsed.year() <= (/* @__PURE__ */ new Date()).getFullYear() + 10) {
        return parsed.format("DD/MM/YYYY");
      }
    }
    const ddmmParsed = moment__default.default(dateStr, "DD/MM/YYYY", true);
    ...
    const mmddParsed = moment__default.default(dateStr, "MM/DD/YYYY", true);
