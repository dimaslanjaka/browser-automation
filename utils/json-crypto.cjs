...
function encryptJson(data, secret) {
  ...
  return CryptoJS__default.default.AES.encrypt(jsonString, secret).toString();
}
function decryptJson(encrypted, secret) {
  const bytes = CryptoJS__default.default.AES.decrypt(encrypted, secret);
  const jsonString = bytes.toString(CryptoJS__default.default.enc.Utf8);
  return jsonParseWithCircularRefsBrowser(jsonString);
}

exports.decryptJson = decryptJson;
exports.encryptJson = encryptJson;
//# sourceMappingURL=json-crypto.cjs.map
//# sourceMappingURL=json-crypto.cjs.map