...
async function action(browser) {
    ...
    await require$$4.delay(5000); // Keep the browser open for a while to observe
    return browser;
}
async function main() {
    const browsers = [];
    ...
    try {
        await Bluebird.map(Array.from({ length: parallelRuns }), async () => {
