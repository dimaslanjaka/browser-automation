...
skrinCheckData.parallelSkrinCheck({ specificNiks, force, openScreenshots, limit, fromDate, toDate })
    .catch((err) => {
    console.error(err);
    process.exit(1);
})
    ...
    skrinCheckData.parallelSkrinCheckEndpointManager.releaseEndpointClaim(skrinCheckData.getParallelSkrinCheckClaimedEndpoint(), process.pid);
    process.exit(0);
});
