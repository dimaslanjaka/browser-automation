...
function colorize(value) {
  ...
  if (value === void 0) return colors.gray("undefined");
  ...
  switch (type) {
    ...
    case "object":
      if (Array.isArray(value)) {
        ...
        const entries = Object.entries(value).map(([k, v]) => `${colors.cyan(k)}: ${colorize(v)}`).join(", ");
        return `{ ${entries} }`;
      }
    default:
...
export { logger_default as default, error, info, log, warn };
