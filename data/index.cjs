...
var url = require('url');
var skrinUtils = require('../src/runner/skrin-utils.cjs');
var date = require('../src/utils/date.cjs');
var jsonCrypto = require('../src/utils/json-crypto.cjs');
var require$$4 = require('sbg-utility');

var _documentCurrentScript = typeof document !== 'undefined' ? document.currentScript : null;
const __filename$1 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('data/index.cjs', document.baseURI).href)));
...
async function loadCsvData(customCsvPath) {
    ...
    if (!fs.existsSync(targetCsvPath)) {
        require$$4.writefile(targetCsvPath, `# Comment
    ...
    const results = await new Promise((resolve, reject) => {
        ...
        fs.createReadStream(targetCsvPath)
            ...
            const dataKunto = mappedRecords.map((row) => {
                ...
                row.tgl_lahir = date.dateStringToDDMMYYYY(row.tgl_lahir);
                return row;
            });
            const outputDir = path.join(process.cwd(), 'public/assets/data');
            fs.mkdirSync(outputDir, { recursive: true });
            fs.writeFileSync(path.join(outputDir, 'dataKunto.bin'), jsonCrypto.encryptJson(dataKunto, process.env.VITE_JSON_SECRET));
            resolve(dataKunto);
        })
            .on('error', reject);
    });
...
if (process.argv.some((arg) => path.toUnix(arg).endsWith('data/index.js')) &&
    ...
    (async () => {
        ...
        console.log(aneh
            .map((item) => `${item.nama} -> ${skrinUtils.parseBabyName(item.nama) ? ansiColors.greenBright(skrinUtils.parseBabyName(item.nama)) : ansiColors.gray('undefined')}`)
            .join('\n'));
    })();
}

exports.loadCsvData = loadCsvData;
exports.normalizeCsvKey = normalizeCsvKey;
exports.normalizeEmptyKeys = normalizeEmptyKeys;
