...
async function selectDayFromCalendar(page, dateStr) {
  const day = moment__default.default(dateStr, "DD/MM/YYYY", true).date().toString();
...
async function selectCalendar(page, dateStr) {
  const m = moment__default.default(dateStr, "DD/MM/YYYY", true);
  if (!m.isValid()) {
  ...
  for (let safety = 0; safety < 600; safety++) {
    ...
    await puppeteer_utils_js.waitForDomStable(page, 150);
...
async function screenshot(page, filePath = "tmp/screenshot.png") {
  fs__default.default.ensureDirSync(path__default.default.dirname(filePath));
...
async function _login(page, options = {}) {
  ...
  if (clearCookies) {
    await puppeteer_utils_js.clearCurrentPageCookies(page);
    await page.goto("https://sehatindonesiaku.kemkes.go.id/auth/login", { waitUntil: "networkidle2" });
  }
...
exports.clickDaftarBaru = clickDaftarBaru;
exports.clickKembali = clickKembali;
exports.enterSehatIndonesiaKu = enterSehatIndonesiaKu;
exports.screenshot = screenshot;
exports.selectCalendar = selectCalendar;
exports.selectDayFromCalendar = selectDayFromCalendar;
//# sourceMappingURL=sehatindonesiaku-utils.cjs.map
//# sourceMappingURL=sehatindonesiaku-utils.cjs.map