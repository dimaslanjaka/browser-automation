...
var sehatindonesiakuKehadiran_js = require('./sehatindonesiaku-kehadiran.js');
var sehatindonesiakuRegistrasi_js = require('./sehatindonesiaku-registrasi.js');
...
async function main() {
  var _a, _b, _c, _d;
  await crossSpawn.spawnAsync("chcp", ["65001"]).catch(utilsBrowser_js.noop);
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    try {
      ...
      await puppeteer_utils_js.closeFirstTab(browser);
      ...
      await sehatindonesiakuRegistrasi_js.processRegistrasiData(await browser.newPage(), item, db);
      console.log(`\u2705 ${item.nik} - ${ansiColors__default.default.green("Successfully registered")}`);
      await puppeteer_utils_js.closeFirstTab(browser);
      console.log(`\u{1F4DD} ${item.nik} - Processing attendance`);
      await sehatindonesiakuKehadiran_js.processKehadiranData(await browser.newPage(), item, db);
      console.log(`\u2705 ${item.nik} - ${ansiColors__default.default.green("Successfully processed attendance")}`);
      await puppeteer_utils_js.closeFirstTab(browser);
    } catch (e) {
      const message = ((dbItem == null ? void 0 : dbItem.message) ?? "").split(",");
      if (e instanceof AlreadyHadir) {
        continue;
      } else if (e instanceof sehatindonesiakuErrors_js.ErrorDataKehadiranNotFound) {
        ...
        continue;
      } else if (e instanceof sehatindonesiakuErrors_js.DataTidakSesuaiKTPError) {
        console.warn(`${item.nik} - ${ansiColors__default.default.red("Data tidak sesuai KTP")}`);
        ...
        needLogin = true;
        console.warn(
          `${ansiColors__default.default.redBright("Login required")}, please ${ansiColors__default.default.bold("login manually")} from opened browser. (close browser manual)`
        );
        break;
      } else if (e instanceof sehatindonesiakuErrors_js.TanggalPemeriksaanError) {
        console.warn(`${item.nik} - ${ansiColors__default.default.red("Tanggal Pemeriksaan tidak valid")}: ${item.tanggal_pemeriksaan}`);
        ...
        continue;
      } else if (e instanceof mariadb.SqlError) {
...
async function checkLoginStatus(page) {
  if (!await sehatindonesiakuUtils_js.enterSehatIndonesiaKu(page)) {
    throw new sehatindonesiakuErrors_js.UnauthorizedError();
  }
}
async function checkRegisteredStatus(page, item, db) {
  ...
  await puppeteer_utils_js.waitForDomStable(page, 500, 1e4);
  await sehatindonesiakuKehadiran_js.searchNik(page, item.nik);
  if (await sehatindonesiakuKehadiran_js.checkAlreadyHadir(page, item, db)) {
    const dbItem = await db.getLogById(item.nik) ?? {};
    ...
    await db.addLog({
      ...
      message: sbgUtility.array_unique(messages).join(",")
    });
...
//# sourceMappingURL=sehatindonesiaku-index.cjs.map
//# sourceMappingURL=sehatindonesiaku-index.cjs.map