...
var SharedPreferences = require('sbg-utility/dist/utils/SharedPreferences');
var path = require('upath');
var xlsx = require('xlsx');
var LogDatabase_js = require('../database/LogDatabase.js');
var googleSheet_js = require('../utils/googleSheet.js');
...
var SharedPreferences__default = /*#__PURE__*/_interopDefault(SharedPreferences);
var path__default = /*#__PURE__*/_interopDefault(path);
var xlsx__default = /*#__PURE__*/_interopDefault(xlsx);

const { MYSQL_HOST, MYSQL_USER, MYSQL_PASS, MYSQL_PORT } = process.env;
let sehatindonesiakuDb = new LogDatabase_js.LogDatabase("sehatindonesiaku-kemkes", {
  connectTimeout: 6e4,
  connectionLimit: 10,
...
async function getSehatIndonesiaKuDb() {
  if (!sehatindonesiakuDb) {
    ...
    sehatindonesiakuDb = new LogDatabase_js.LogDatabase("sehatindonesiaku-kemkes", {
      connectTimeout: 6e4,
      connectionLimit: 10,
      host: MYSQL_HOST || "localhost",
      user: MYSQL_USER || "root",
...
const sehatindonesiakuPref = new SharedPreferences__default.default({ namespace: "sehatindonesiaku-kemkes" });
const xlsxFile = path__default.default.join(process.cwd(), ".cache/sheets/sehatindonesiaku.xlsx");
...
async function parseXlsxFile(filePath = xlsxFile, rangeIndex = 6, rangeEndIndex = Number.MAX_SAFE_INTEGER) {
  const workbook = xlsx__default.default.readFile(filePath);
  ...
  if (ref) {
    ...
    maxRow = xlsx__default.default.utils.decode_cell(endCell).r;
  }
  const effectiveEndIndex = Math.min(rangeEndIndex, maxRow);
  if (typeof rangeEndIndex === "number" && effectiveEndIndex >= rangeIndex) {
    if (ref) {
      const [, endCell] = ref.split(":");
      const startCell = xlsx__default.default.utils.encode_cell({ c: 0, r: rangeIndex });
      const endCol = xlsx__default.default.utils.decode_cell(endCell).c;
      const endCellStr = xlsx__default.default.utils.encode_cell({ c: endCol, r: effectiveEndIndex });
      sheetToJsonRange = `${startCell}:${endCellStr}`;
    }
  }
  const data = xlsx__default.default.utils.sheet_to_json(sheet, {
    header: 1,
    defval: null,
...
const outPath = path__default.default.join(process.cwd(), ".cache/sheets/sehatindonesiaku-data.json");
async function downloadAndProcessXlsx(options = {}) {
  ...
  const downloadResult = await googleSheet_js.downloadSheets(resolvedSpreadsheetId, !cache);
  const result = await parseXlsxFile(downloadResult.xlsxFilePath, rangeIndex, rangeEndIndex);
  fs__default.default.ensureDirSync(path__default.default.dirname(outPath));
  fs__default.default.writeFileSync(outPath, JSON.stringify(result, null, 2), "utf-8");
  console.log(`Parsed XLSX data (Format Full) written to: ${outPath}`);
}
async function getExcelData() {
  const rawData = JSON.parse(fs__default.default.readFileSync(outPath, "utf-8"));
  return rawData;
}
...
exports.closeSehatIndonesiaKuDb = closeSehatIndonesiaKuDb;
exports.downloadAndProcessXlsx = downloadAndProcessXlsx;
exports.fixKemkesDataItem = fixKemkesDataItem;
exports.getDbData = getDbData;
exports.getExcelData = getExcelData;
exports.getSehatIndonesiaKuDb = getSehatIndonesiaKuDb;
exports.parseXlsxFile = parseXlsxFile;
exports.restartSehatIndonesiaKuDb = restartSehatIndonesiaKuDb;
exports.sehatindonesiakuDataPath = outPath;
exports.sehatindonesiakuPref = sehatindonesiakuPref;
exports.showHelp = showHelp;
//# sourceMappingURL=sehatindonesiaku-data.cjs.map
//# sourceMappingURL=sehatindonesiaku-data.cjs.map